...
//Consulta multitabla  
      $query = mysqli_query($conexion, "SELECT tb.id_TTHH,p.nombres , a.unidad_administrador, r.nombre_Rol,mt.tipo_Motivo, 
        fp.desde, fp.hasta, fp.nro_horas, IF(tb.estado=1,'Espera','Aceptado') as estado FROM 
        tabla_central tb, personal p, formurario_permiso fp, areas a, roles r, motivo mt
        WHERE  tb.id_permisos=fp.id_formurario_permiso 
        and p.id_personal=tb.id_personal and fp.fk_motivo_permiso=mt.id_motivo
        and p.fk_rol=r.id_roles and fp.fk_unidad_administrativa=$id_areas2
        and a.id_areas=$id_areas2 and tb.estado=1
        union
        SELECT tb.id_TTHH,p.nombres , a.unidad_administrador, r.nombre_Rol,mt.tipo_Motivo, 
        fp.desde, fp.hasta, fp.nro_horas, IF(tb.estado=0,'Aceptado','Rechazado') as estado FROM 
        tabla_central tb, personal p, formurario_permiso fp, areas a, roles r, motivo mt
        WHERE  tb.id_permisos=fp.id_formurario_permiso 
        and p.id_personal=tb.id_personal and fp.fk_motivo_permiso=mt.id_motivo
        and p.fk_rol=r.id_roles and fp.fk_unidad_administrativa=$id_areas2
        and a.id_areas=$id_areas2 and (tb.estado=0 or tb.estado=2) 
